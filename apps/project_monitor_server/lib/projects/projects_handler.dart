...
ProjectFields _decodeFields(JsonDecoder decoder) =>
    ProjectFields(name: decoder.field('name'));
...
Future<Response> _updateProject(ProjectsRepo repo, Request request) async {
  return Responses.json({'error': 'Not implemented'}, code: 503);
}

Future<Response> _deleteProject(ProjectsRepo repo, Request request) async {
  return Responses.json({'error': 'Not implemented'}, code: 503);
}
...
Handler projectsHandler(ProjectsRepo repo) {
  final router = Router()
    ...
    ..put('/<id>', (Request request, String id) => _updateProject(repo, request))
    ..delete('/<id>', (Request request, String id) => _deleteProject(repo, request));
