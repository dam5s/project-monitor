...
class TestGithubServer {
  ...
  Future<void> _start() async {
    ...
    _server = await io.serve(_app, 'localhost', 0);
  }
  ...
  Response _buildWorkflowRunsResponse(String owner, String name) {
    final matchingRuns = _stubbedWorkflowRuns.where((r) => r.project.repoOwner == owner && r.project.repoName == name);
...
final class StubbedWorkflowRun {
  ...
  StubbedWorkflowRun({required this.project, required this.id, required this.name, required this.head_branch, required this.status, required this.url});

  dynamic toJson() {
    return {'id': id, 'name': name, 'head_branch': head_branch, 'status': status, 'url': url};
  }
}